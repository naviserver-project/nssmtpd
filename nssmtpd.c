        ...
        if (!strncasecmp(conn->line.string, "RCPT TO:", 8)) {
            ...
            if (parseEmail(&addr, data)) {
                ...
                if (SmtpdCheckRelay(conn, &addr, &host, &port))
                    flags |= SMTPD_RELAY;
                else if (!(conn->flags & SMTPD_LOCAL)) {
...
static smtpdIpaddr *SmtpdParseIpaddr(char *str)
{
    ...
    else if (sscanf(str, "%[^/]/%s", addr, mask) == 2);
    else if (sscanf(str, "%s", addr) == 1) {
        int hp_errno;
        char **x, buf[1024];
        smtpdIpaddr *arec;
    ...
    if (*mask) {
        ...
        else if ((ipmask = atoi(mask)) >= 0 && ipmask < 33)
...
static int SmtpdCheckVirus(smtpdConn * conn, char *data, int datalen, char *location)
{
    ...
    if (datalen) {
        ...
        if (cl_scanfile
            (data, &virname, &size, conn->server->ClamAvRoot, &conn->server->ClamAvLimits,
             CL_ARCHIVE | CL_MAIL | CL_OLE2) == CL_VIRUS) {
            conn->flags |= SMTPD_GOTVIRUS;
            SmtpdConnAddHeader(conn, SMTPD_HDR_VIRUS_STATUS, virname, 1);
        }
    }
...
static int SmtpdCmd(ClientData arg, Tcl_Interp * interp, int objc, Tcl_Obj * CONST objv[])
{
    ...
    case cmdSend:{
            ...
            if (SmtpdSend(server,
                          interp, Tcl_GetString(objv[2]), Tcl_GetString(objv[3]), Tcl_GetString(objv[4]), host, port))
                return TCL_ERROR;
            break;
        }
