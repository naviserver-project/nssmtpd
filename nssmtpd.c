...
static char *encodehex(char *buf,int len);
static char *decodehex(char *str,int *len);
...
static dnsRecord *dnsRecordCreateA(char *name,unsigned long ipaddr);
...
static void dnsEncodeObj(dnsPacket *pkt);
...
static dnsPacket *dnsPacketCreateQuery(char *name,int type);
static void dnsPacketFree(dnsPacket *pkt,int type);
static int dnsPacketAddRecord(dnsPacket *pkt,dnsRecord **list,short *count,dnsRecord *rec);
...
static void SmtpdPanic(char *fmt,...);
...
Ns_ModuleInit(char *server, char *module)
{
    ...
    if (!Ns_ConfigGetInt(path,"debug",&serverPtr->debug)) serverPtr->debug = 1;
    if (!Ns_ConfigGetInt(path,"readtimeout",&serverPtr->readtimeout)) serverPtr->readtimeout = 60;
    if (!Ns_ConfigGetInt(path,"writetimeout",&serverPtr->writetimeout)) serverPtr->writetimeout = 60;
    if (!Ns_ConfigGetInt(path,"bufsize",&serverPtr->bufsize)) serverPtr->bufsize = 1024*4;
    if (!Ns_ConfigGetInt(path,"maxrcpt",&serverPtr->maxrcpt)) serverPtr->maxrcpt = 100;
    if (!Ns_ConfigGetInt(path,"maxline",&serverPtr->maxline)) serverPtr->maxline = 4096;
    if (!Ns_ConfigGetInt(path,"maxdata",&serverPtr->maxdata)) serverPtr->maxdata = 1024*1024*10;
    ...
    if (serverPtr->flags & SMTPD_SEGV) {
      if (!Ns_ConfigGetInt(path,"segvtimeout",&segvTimeout)) segvTimeout = -1;
...
#ifdef USE_SAVI
    {
      ...
      if (pSAVI->pVtbl->GetVirusEngineVersion(pSAVI,
                                             ...
                                             NULL) >= 0)
        Ns_Log(Notice,"nssmtpd: sophos: Engine version %d.%d %s, Number of detectable viruses: %u, Date of virus data: %d/%d/%d",
               (int)version>>16,(int)version&0x0000FFFF,versionString,
               (unsigned)detectableViruses,vdlDate.wMonth,vdlDate.wDay,vdlDate.wYear);
    }
    ...
    {
      int rc,virnum;
      if (!(addr = Ns_ConfigGetValue(path,"clamav_dbdir"))) addr = cl_retdbdir();
      ...
      if (!Ns_ConfigGetInt(path,"clamav_maxfiles",&serverPtr->ClamAvLimits.maxfiles)) serverPtr->ClamAvLimits.maxfiles = 1000;
      if (!Ns_ConfigGetInt(path,"clamav_maxfilesize",&serverPtr->ClamAvLimits.maxfilesize)) serverPtr->ClamAvLimits.maxfilesize = 10 * 1048576;
      if (!Ns_ConfigGetInt(path,"clamav_maxreclevel",&serverPtr->ClamAvLimits.maxreclevel)) serverPtr->ClamAvLimits.maxreclevel = 5;
      if (!Ns_ConfigGetInt(path,"clamav_maxratio",&serverPtr->ClamAvLimits.maxratio)) serverPtr->ClamAvLimits.maxratio = 200;
      if (!Ns_ConfigGetInt(path,"clamav_archivememlim",&serverPtr->ClamAvLimits.archivememlim)) serverPtr->ClamAvLimits.archivememlim = 0;
...
SmtpdPanic(char *fmt,...)
{
    ...
    va_end(ap);
    while(time(0) - now < segvTimeout) sleep(1);
    kill(getpid(),SIGKILL);
}
 ...
 */
static int
SmtpdInterpInit(Tcl_Interp *interp, void *arg)
{
...
static int
SmtpdRequestProc(void *arg, Ns_Conn *conn)
{
...
static int
SmtpdProc(Ns_DriverCmd cmd, Ns_Sock *sock, struct iovec *bufs, int nbufs)
{
    switch (cmd) {
     case DriverAccept:
...
{
    smtpdServer *server = arg;
    if (server->initproc) Ns_TclEval(0,server->server,server->initproc);
}

static void
SmtpdThread(smtpdConn *conn)
{
    char *data;
    ...
    if (SmtpdWriteDString(conn,&conn->line)) goto error;

    while(1) {
      conn->cmd = SMTP_READ;
      if (SmtpdReadLine(conn,&conn->line) < 0) goto error;
      ...
      if (!strncasecmp(conn->line.string,"HELP",4)) {
        ...
        if (SmtpdPuts(conn,ds.string) != NS_OK) goto error;
        Ns_DStringFree(&ds);
        continue;
      }
      
      if (!strncasecmp(conn->line.string,"HELO",4) ||
         !strncasecmp(conn->line.string,"EHLO",4)) {
        conn->cmd = SMTP_HELO;
        ...
        if (conn->reply.length) {
          ...
          if (!strncasecmp(conn->line.string,"HELO",4)) {
            ...
            if (SmtpdWriteDString(conn,&conn->line) != NS_OK) goto error;
          }
        }
        if (conn->flags & SMTPD_ABORT) break;
      ...
      if (!strncasecmp(conn->line.string,"RCPT TO:",8)) {
        ...
        conn->rcpt.list = rcpt;
        if (rcpt->next) rcpt->next->prev = rcpt;
        conn->rcpt.count++;
        /* Call Tcl callback */
      ...
      if (!strncasecmp(conn->line.string,"DATA",4)) {
        ...
        if (server->relayhost) {
          for (rcpt = conn->rcpt.list;rcpt;rcpt = rcpt->next)
            if (rcpt->flags & SMTPD_VERIFIED) break;
          if (rcpt && (size = SmtpdRelayData(conn,rcpt->relay.host,rcpt->relay.port)) < 0) goto done;
        }
        ...
        if (size == -1) {
          ...
          do {
            ...
            if (size < server->maxdata) Ns_DStringNAppend(&conn->body.data,conn->line.string,conn->line.length);
        ...
        if (server->relayhost) {
          for (rcpt = conn->rcpt.list;rcpt;rcpt = rcpt->next)
            if (!(rcpt->flags & SMTPD_VERIFIED)) break;
          if (!rcpt) Ns_DStringTrunc(&conn->reply,0);
        }
        if (SmtpdWriteDString(conn,&conn->reply) != NS_OK) goto error;
        if (conn->flags & SMTPD_ABORT) break;
        SmtpdConnPrint(conn);
    ...
    switch (errno) {
     ...
     default:
         Ns_Log(Error,"nssmtpd: %d/%d: HOST: %s/%s, I/O error: %d/%d: %s: %s",
                ...
                conn->sock->sock,conn->cmd,
                strerror(errno),conn->line.string);
    }
    SmtpdConnFree(conn);
}

static smtpdConn*
SmtpdConnCreate(smtpdServer *server,Ns_Sock *sock)
{
    smtpdConn *conn;
    Tcl_HashEntry *rec;
    int new;
    ...
    if ((conn = connList)) connList = connList->next;
    ...
    conn->id = server->id++;
    rec = Tcl_CreateHashEntry(&server->sessions,(char*)conn->id,&new);
...
static void
SmtpdConnPrint(smtpdConn *conn)
{
    Ns_Conn *nsconn;
    smtpdRcpt *rcpt;

    if (conn->server->debug < 1) return;

    nsconn = Ns_GetConn();
    Ns_DStringTrunc(&conn->line,0);
    Ns_DStringPrintf(&conn->line,"nssmtpd: %d/%d: HOST: %s/%s",
       conn->id,getpid(),conn->host,Ns_ConnPeer(nsconn));
...
static int
SmtpdConnEval(smtpdConn *conn,char *proc)
{
    char name[256];

    Ns_DStringTrunc(&conn->reply,0);
    if (!proc || !*proc) return TCL_OK;
...
SmtpdConnFree(smtpdConn *conn)
{
    ...
    if ((rec = Tcl_FindHashEntry(&conn->server->sessions,(char*)conn->id)))
...
SmtpdRelayData(smtpdConn *conn,char *host,int port)
{
    ...
    if (SmtpdWriteDString(relay,&conn->line) != NS_OK) goto error421;
    if (SmtpdReadLine(relay,&relay->line) <= 0) goto error421;
    if (relay->line.string[0] != '2') goto errorrelay;
    ...
    /* RCPT TO command */
    for (rcpt = conn->rcpt.list;rcpt;rcpt = rcpt->next) {
      if (!(rcpt->flags & SMTPD_VERIFIED)) continue;
      Ns_DStringTrunc(&conn->line,0);
    ...
    if (SmtpdReadLine(relay,&relay->line) <= 0) goto error421;
    if (strncmp(relay->line.string,"354",3)) goto errorrelay;
    if (SmtpdPuts(conn,"354 Start mail input; end with <CRLF>.<CRLF>\r\n") != NS_OK) goto error;
    do {
      if (SmtpdReadLine(conn,&relay->line) < 0) goto error;
      if (SmtpdWriteDString(relay,&relay->line) != NS_OK) goto error421;
    ...
    if (SmtpdReadLine(relay,&relay->line) <= 0) goto error421;
    if (relay->line.string[0] != '2') goto errorrelay;
    if (SmtpdWriteDString(conn,&relay->line) != NS_OK) goto error;

    SmtpdConnFree(relay);
    for (rcpt = conn->rcpt.list;rcpt;rcpt = rcpt->next)
      if (rcpt->flags & SMTPD_VERIFIED) rcpt->flags |= SMTPD_DELIVERED;
    return size;
...
error421:
    ...
    Ns_Log(Error,"nssmtpd: relay: %d/%d: HOST: %s/%s, FLAGS: 0x%X, FROM: %s, %s: %s/%s",
           conn->id,getpid(),conn->host,Ns_ConnPeer(nsconn),
           conn->flags,conn->from.addr,conn->line.string,
...
static int
SmtpdSend(smtpdServer *server,Tcl_Interp *interp,const char *sender,const char *rcpt,char *dname,char *host,int port)
{
    char *ptr;
    Ns_Sock sock;
    Tcl_Obj *data;
    ...
    if (!(data = Tcl_GetVar2Ex(interp,dname,0,TCL_LEAVE_ERR_MSG))) return -1;
    ...
    Ns_DStringTrunc(&conn->line,0);
    Ns_DStringPrintf(&conn->line,"HELO %s\r\n",Ns_InfoHostname());
    if (SmtpdWriteDString(conn,&conn->line) != NS_OK) goto ioerror;
    if (SmtpdReadLine(conn,&conn->line) <= 0) goto ioerror;
    if (conn->line.string[0] != '2') goto error;
    ...
    if (SmtpdReadLine(conn,&conn->line) <= 0) goto ioerror;
    if (strncmp(conn->line.string,"354",3)) goto error;
    if (SmtpdWriteData(conn,Tcl_GetString(data),(int)Tcl_GetCharLength(data)) != NS_OK) goto ioerror;
    if (SmtpdWriteData(conn,"\r\n.\r\n",5) != NS_OK) goto ioerror;
    if (SmtpdReadLine(conn,&conn->line) <= 0) goto ioerror;
    if (conn->line.string[0] != '2') goto error;
    ...
    if (duplicated) Tcl_DecrRefCount(data);
    return 0;

ioerror:
    if (errno) Tcl_AppendResult(interp,"nssmtpd: send: I/O error: ",conn->line.string,": ",strerror(errno),0);
    SmtpdConnFree(conn);
    if (duplicated) Tcl_DecrRefCount(data);
    return -1;
...
static void
SmtpdRcptFree(smtpdConn *conn,char *addr,int index,int flags)
{
    int count = -1;
    ...
    for (rcpt = conn->rcpt.list;rcpt;) {
      count++;
      if ((flags && rcpt->flags & SMTPD_VERIFIED) ||
         (addr && !strcmp(rcpt->addr,addr)) ||
         (index >= 0 && count == index)) {
        if (rcpt->prev) rcpt->prev->next = rcpt->next; else conn->rcpt.list = rcpt->next;
        if (rcpt->next) rcpt->next->prev = rcpt->prev;
        rcpt2 = rcpt;
        rcpt = rcpt->next;
        ns_free(rcpt2->addr);
        ns_free(rcpt2->data);
...
SmtpdRead(smtpdConn *conn, void *vbuf, int len)
{
	...
	/* Copy bytes already in read-ahead buffer. */
        if (conn->buf.pos > len) n = len; else n = conn->buf.pos;
...
static int
SmtpdWrite(smtpdConn *conn, void *vbuf, int len)
{
    int nwrote,n;
    char *buf;
    Ns_Time timeout = { conn->server->writetimeout, 0};
...
SmtpdReadLine(smtpdConn *conn,Ns_DString *dsPtr)
{
    ...
    if (conn->server->debug > 5 && nread > 0)  Ns_Log(Notice,"nssmtpd: %d: <<< %s",conn->id,dsPtr->string);
    return (nread > 0 ? len : nread);
}

static int
SmtpdWriteData(smtpdConn *conn,char *buf,int len)
{
    int nwrote;
    if (conn->server->debug > 5) Ns_Log(Notice,"nssmtpd: %d: >>> %s",conn->id,buf);
    while(len > 0) {
      nwrote = SmtpdWrite(conn,buf,len);
      if (nwrote < 0) return NS_ERROR;
      len -= nwrote;
      buf += nwrote;
    }
    return NS_OK;
...
static int
SmtpdWriteDString(smtpdConn *conn,Ns_DString *dsPtr)
{
    return SmtpdWriteData(conn,dsPtr->string,dsPtr->length);
}

static int
SmtpdPuts(smtpdConn *conn,char *string)
{
    return SmtpdWriteData(conn,string,(int)strlen(string));
}

static char*
SmtpdStrPos(char *as1, char *as2)
{
...
static char*
SmtpdStrNPos(char *as1, char *as2, int len)
{
...
static char*
SmtpdStrTrim(char *str)
{
    int len;

    while(*str == '<' || isspace(*str)) str++;
    len = strlen(str);
    while(len-- && (isspace(str[len]) || str[len] == '>')) str[len] = 0;
    return str;
}

static char *
SmtpdGetHeader(smtpdConn *conn,char *name)
{
   smtpdHdr *hdr;
   for (hdr = conn->body.headers;hdr;hdr = hdr->next)
     if (!strcasecmp(name,hdr->name) && hdr->value && *hdr->value) return hdr->value;
   return "";
}

static void
SmtpdConnAddHeader(smtpdConn *conn,char *name,char *value,int alloc)
{
 ...
 */
static void
SmtpdConnParseData(smtpdConn *conn)
{
    ...
    while(end && end <= body) {
      ...
      if (*(end-1) == '\r') end--;
      while(line < end && *line >= 33 && *line <= 126 && *line != ':') line++;
      ...
      if (line < end) {
        ...
        if (!strcasecmp(header->name,"Sender") ||
           ...
           !strcasecmp(header->name,"Reply-To")) {
          smtpdEmail addr;
          Ns_DStringTrunc(&conn->reply,0);
        ...
        if (end == body) {
          ...
          if (fileHdr && encodingType && (conn->flags & SMTPD_VIRUSCHECK)) {
            ...
            if (conn->flags & SMTPD_GOTVIRUS) {
              while(body[size-1] == '\n' || body[size-1] == '\r') size--;
              memset(body,' ',size);
              if (size > strlen(info)) memcpy(body,info,strlen(info));
              if (strlen(fileHdr->value) > 4) memcpy(filePtr+strlen(fileHdr->value)-4,".txt",4);
              memcpy(encodingType,"8bit",4);
              memset(encodingType+4,' ',encodingSize-4);
              memset(contentType,' ',contentSize);
              if (contentSize >= 10) memcpy(contentType,"text/plain",10);
              Ns_Log(Notice,"nssmtpd: %d/%d: virus detected: %s",conn->id,getpid(),SmtpdGetHeader(conn,SMTPD_HDR_VIRUS_STATUS));
            }
          }
#endif
          break;
...
static smtpdIpaddr*
SmtpdParseIpaddr(char *str)
{
    smtpdIpaddr *alist = 0;
    char addr[32],mask[32] = "";
    unsigned long ipaddr,ipmask = 0;

    if (sscanf(str,"%[0123456789.]/%[0123456789.]",addr,mask) == 2) ; else
    if (sscanf(str,"%[0123456789.]",addr) == 1) ; else
    if (sscanf(str,"%[^/]/%s",addr,mask) == 2) ; else
...
#if defined(linux)
      if (!gethostbyname_r(addr,&hp_a,buf,sizeof(buf),&hp_b,&hp_errno)) hp = &hp_a;
#else
    ...
    if ((ipaddr = inet_addr(addr)) < 0) return 0;
    /* Decode mask */
    if (*mask) {
      if (strchr(mask,'.'))
    ...
    if (!ipmask) {
      ipmask = ntohl(ipaddr);
      if (!(ipmask & 0xFFFFFFFFul)) ipmask = htonl(0x00000000ul); else
      if (!(ipmask & 0x00FFFFFF)) ipmask = htonl(0xFF000000ul); else
      if (!(ipmask & 0x0000FFFF)) ipmask = htonl(0xFFFF0000ul); else
      if (!(ipmask & 0x000000FF)) ipmask = htonl(0xFFFFFF00ul); else
...
static int
SmtpdCheckDomain(smtpdConn *conn,char *domain)
{
    dnsRecord *rec;
    dnsPacket *reply;

    if (conn && !(conn->flags & SMTPD_NEEDDOMAIN)) return 1;
    ...
    if (!conn) return 0;
    Ns_Log(Error,"nssmtpd: checkdomain: %d: HOST: %s, FLAGS: 0x%X, %s",
           conn->id,conn->host,conn->flags,domain);
    Ns_DStringPrintf(&conn->reply,"553 %s... Domain unrecognised\r\n",domain);
    return 0;
}

static int
SmtpdCheckRelay(smtpdConn *conn,smtpdEmail *addr,char **host,int *port)
{
    char *p,*s;
    smtpdRelay *relay;
      ...
      while(*p == *s) {
        if (s-- == relay->name) {
          ...
          if (p == addr->domain || *(p-1) == '.') {
            if (host) *host = ns_strcopy(relay->host);
...
SmtpdCheckSpam(smtpdConn *conn)
{
    ...
    for (rcpt = conn->rcpt.list;rcpt;rcpt = rcpt->next)
      if (!(rcpt->flags & SMTPD_DELIVERED) && (rcpt->flag & SMTPD_SPAMCHECK)) break;
    if (!rcpt) return 0;
    ...
    if (SmtpdWriteDString(spamd,&spamd->line) != NS_OK) goto error;
    if (SmtpdWriteDString(spamd,&conn->body.data) != NS_OK) goto error;
    shutdown(sock.sock,1);
    if (SmtpdReadLine(spamd,&spamd->line) < 0) goto error;
    /* We should receive 0 response code */
    if (strncasecmp(spamd->line.string,"SPAMD/",6) || !strstr(spamd->line.string,"EX_OK")) goto error;
    if (SmtpdReadLine(spamd,&spamd->line) < 0) goto error;
    /* Validate spam line */
    if (strncmp(spamd->line.string,"Spam:",5) || !(p = strchr(spamd->line.string+6,';'))) goto error;
    rc = strstr(spamd->line.string,"True") ? SMTPD_GOTSPAM : 0;
    score = atof(++p);
    // Update all recipients with spam score/status
    for (;rcpt;rcpt = rctp->next) {
      if (rcpt->flags & SMTPD_DELIVERED || !(rcpt->flag & SMTPD_SPAMCHECK)) continue;
      rcpt->flags |= rc;
    ...
    return 1;
error:
    Ns_Log(Error,"nssmtpd: spam: %d/%d: %s/%s: %s",conn->id,getpid(),spamd->line.string,spamd->reply.string,strerror(errno));
    SmtpdConnFree(spamd);
    return -1;
#endif
    ...
    for (rcpt = conn->rcpt.list;rcpt;rcpt = rcpt->next) {
      if (rcpt->flags & SMTPD_DELIVERED || !(rcpt->flags & SMTPD_SPAMCHECK)) continue;
      if (!(CTX = dspam_init(rcpt->addr,NULL,DSM_PROCESS,DSF_SIGNATURE|DSF_CHAINED|DSF_NOISE))) goto error;
...
error:
    ...
    if (CTX) _ds_destroy_message(CTX->message),dspam_destroy(CTX);
    return -1;
#endif
    return 0;
}
...
SmtpdCheckVirus(smtpdConn *conn,char *data,int datalen,char *location)
{
    ...
    if (!location) location = datalen ? "buffer" : data;
    ...
    while(pEnumResults->pVtbl->Next(pEnumResults,1,(void **)&pResults,&pcFetched) == SOPHOS_S_OK) {
      if (pResults->pVtbl->GetVirusType(pResults,&virusType) < 0 ||
         virusType == SOPHOS_NO_VIRUS) break;
      switch (virusType) {
       case SOPHOS_VIRUS:
    ...
    Ns_DStringFree(&ds);
    if (pResults) pResults->pVtbl->Release(pResults);
...
static smtpdIpaddr*
SmtpdCheckIpaddr(smtpdIpaddr *list,unsigned long addr)
{
    while(list) {
      if (ntohl(addr & list->mask.s_addr) == ntohl(list->addr.s_addr)) return list;
      list = list->next;
    }
    return 0;
}
...
static int
SmtpdFlags(char *name)
{
    if (!strcasecmp(name,"verified")) return SMTPD_VERIFIED;
    if (!strcasecmp(name,"local")) return SMTPD_LOCAL;
    if (!strcasecmp(name,"relay")) return SMTPD_RELAY;
    if (!strcasecmp(name,"delivered")) return SMTPD_DELIVERED;
    if (!strcasecmp(name,"abort")) return SMTPD_ABORT;
    if (!strcasecmp(name,"needdomain")) return SMTPD_NEEDDOMAIN;
    if (!strcasecmp(name,"segv")) return SMTPD_SEGV;
    if (!strcasecmp(name,"fastproxy")) return SMTPD_FASTPROXY;
    if (!strcasecmp(name,"resolve")) return SMTPD_RESOLVE;
    if (!strcasecmp(name,"needhelo")) return SMTPD_NEEDHELO;
    if (!strcasecmp(name,"gothelo")) return SMTPD_GOTHELO;
    if (!strcasecmp(name,"gotmail")) return SMTPD_GOTMAIL;
    if (!strcasecmp(name,"spamcheck")) return SMTPD_SPAMCHECK;
    if (!strcasecmp(name,"viruscheck")) return SMTPD_VIRUSCHECK;
    if (!strcasecmp(name,"gotspam")) return SMTPD_GOTSPAM;
    if (!strcasecmp(name,"gotvirus")) return SMTPD_GOTVIRUS;
    return 0;
}

static int
SmtpdCmd(ClientData arg,Tcl_Interp *interp,int objc,Tcl_Obj *CONST objv[])
{
    char *name = 0;
    smtpdRcpt *rcpt;
    smtpdConn *conn = 0;
    ...
    if (cmd > cmdSessions) {
      ...
      Ns_MutexLock(&server->lock);
      rec = Tcl_FindHashEntry(&server->sessions,(char*)id);
      Ns_MutexUnlock(&server->lock);
      if (!rec) {
    ...
    switch (cmd) {
     ...
     case cmdDecode:
         ...
         if (!strcmp(Tcl_GetString(objv[2]),"base64")) {
           int len;
           name = (char *)Tcl_GetByteArrayFromObj(objv[3],&count);
           if (!(name = decode64(name,count,&len))) break;
         ...
         if (!strcmp(Tcl_GetString(objv[2]),"qprint")) {
           int len;
           name = (char *)Tcl_GetByteArrayFromObj(objv[3],&count);
           if (!(name = decodeqp(name,count,&len))) break;
           Tcl_SetObjResult(interp,Tcl_NewByteArrayObj((unsigned char*)name,len));
           ns_free(name);
         } else {
     ...
     case cmdInfo:
         ...
         if (!strcasecmp("address",Tcl_GetString(objv[2]))) {
           ...
           if (address) address += 3; else address = server->driver->location;
         ...
         if (!strcasecmp("get",Tcl_GetString(objv[2]))) {
           ...
           for (relay = server->relaylist;relay;relay = relay->next) {
             ...
             if (relay->host) {
               ...
               if (relay->port) Tcl_AppendObjToObj(obj,Tcl_NewIntObj(relay->port));
             }
     ...
     case cmdLocal:
         ...
         if (!strcasecmp("add",Tcl_GetString(objv[2]))) {
           ...
           if ((addr = SmtpdParseIpaddr(Tcl_GetString(objv[3])))) {
             ...
             if (end) end->next = addr; else server->local = addr;
           }
           Ns_MutexUnlock(&server->locallock);
         } else
         if (!strcasecmp("del",Tcl_GetString(objv[2]))) {
     ...
     case cmdSend: {
         ...
         if (objc > 5) host = Tcl_GetString(objv[5]);
         if (objc > 6) port = atoi(Tcl_GetString(objv[6]));
         if (!host || !*host) {
           smtpdEmail addr;
           smtpdConn conn;
     ...
     case cmdGetHdrs: {
         ...
         smtpdHdr *hdr;
         if (objc > 3) name = Tcl_GetString(objv[3]);
         for (hdr = conn->body.headers;hdr;hdr = hdr->next) {
           if (objc > 3) {
             if (!strcasecmp(name,hdr->name) && hdr->value && *hdr->value)
     ...
     case cmdDump: {
         ...
         if ((fp = fopen(Tcl_GetString(objv[3]),"a"))) {
           fprintf(fp,"From: %s\n",conn->from.addr);
           for (rcpt = conn->rcpt.list;rcpt;rcpt = rcpt->next) fprintf(fp,"To: %s\n",rcpt->addr);
           fputs("\n",fp);
     ...
     case cmdSetFlag: {
         ...
         if (index == -1) {
           if (flags > 0) conn->flags |= flags; else conn->flags &= ~(flags*-1);
         ...
         for (count = 0,rcpt = conn->rcpt.list;rcpt;rcpt = rcpt->next,count++) {
           if ((index >= 0 && index == count) || (name && !strcmp(name,rcpt->addr))) {
             if (flags > 0) rcpt->flags |= flags; else rcpt->flags &= ~(flags*-1);
     ...
     case cmdAddRcpt:
         ...
         if (objc > 4 && Tcl_GetIntFromObj(interp,objv[4],&rcpt->flags) != TCL_OK) return TCL_ERROR;
         if (objc > 5) rcpt->data = ns_strcopy(Tcl_GetString(objv[5]));
         rcpt->next = conn->rcpt.list;
         conn->rcpt.list = rcpt;
         conn->rcpt.count++;
         break;
     ...
     case cmdTrainSpam: {
         ...
         if (objc > 5) {
           if ((SIG.data = decodehex(Tcl_GetString(objv[5]),&SIG.length))) flags |= DSF_SIGNATURE;
         }
         if (!(CTX = dspam_init(Tcl_GetString(objv[3]),NULL,DSM_PROCESS,flags))) break;
         CTX->source = DSS_ERROR;
         CTX->classification = atoi(Tcl_GetString(objv[2])) ? DSR_ISSPAM : DSR_ISINNOCENT;
         if (objc > 6) {
           if (!strcmp(Tcl_GetString(objv[6]),"teft")) CTX->training_mode = DST_TEFT; else
           if (!strcmp(Tcl_GetString(objv[6]),"toe")) CTX->training_mode = DST_TOE; else
           if (!strcmp(Tcl_GetString(objv[6]),"tum")) CTX->training_mode = DST_TUM;
         }
         if (objc > 7) {
           if (!strcmp(Tcl_GetString(objv[7]),"error")) CTX->source = DSS_ERROR; else
           if (!strcmp(Tcl_GetString(objv[7]),"corpus")) CTX->source = DSS_CORPUS; else
           if (!strcmp(Tcl_GetString(objv[7]),"inoculation")) CTX->source = DSS_INOCULATION; else
           if (!strcmp(Tcl_GetString(objv[7]),"none")) CTX->source = DSS_NONE;
         }
         if (flags & DSF_SIGNATURE) CTX->signature = &SIG;
         dspam_process(CTX,Tcl_GetString(objv[4]));
         if (flags & DSF_SIGNATURE) ns_free(SIG.data);
         Ns_DStringInit(&ds); 
 ...
 */
static int
parsePhrase(char **inp,char **phrasep,char *specials)
{
    int c;
    char *src = *inp,*dst;
 ...
 */
static int
parseDomain(char **inp,char **domainp,char **commentp)
{
    int c,comment;
 ...
 */
static int
parseRoute(char **inp,char **routep)
{
    int c;
    char *src = *inp,*dst;
	...
	*dst++ = c;
	...
	src = parseSpace(src);
        ...
        while(dst > *routep && (dst[-1] == '.' || dst[-1] == ',' || dst[-1] == '@')) dst--;
        *dst = 0;
        *inp = src;
        return c;
      }
        ...
        if (s - in < len)
         switch (c = *s++) {
          ...
          default:
            ...
            if (isdigit(c)) c -= '0'; else
            c -= (isupper(c) ? 'A' - 10 : 'a' - 10);
            if (isdigit(c2)) c2 -= '0'; else
    ...
    if (!strcmp(name,"nameserver")) {
      ...
      while((s = va_arg(ap,char*))) {
        while(s) {
          ...
          if (!next) dnsServers = server; else next->next = server;
          s = n;
        }
      }
    } else
...
static dnsPacket *
dnsLookup(char *name,int type,int *errcode)
{
    fd_set fds;
    ...
    while(1) {
      ...
      while(retries--) {
        ...
        if (sendto(sock,req->buf.data+2,req->buf.size,0,(struct sockaddr*)&saddr,len) < 0) {
          if (dnsDebug > 3) Ns_Log(Error,"dnsLookup: %s: sendto: %s",server->name,strerror(errno));
          continue;
        }
        tv.tv_usec = 0;
        tv.tv_sec = timeout;
        ...
        if (select(sock+1,&fds,0,0,&tv) <= 0 || !FD_ISSET(sock,&fds)) {
          if (dnsDebug > 3 && errno) Ns_Log(Error,"dnsLookup: %s: select: %s",server->name,strerror(errno));
          continue;
        }
        ...
        if (!(reply = dnsParsePacket((unsigned char*)buf,len))) continue;
...
static void
dnsRecordDestroy(dnsRecord **pkt)
{
    if (!pkt) return;
    while(*pkt) {
...
static dnsRecord *
dnsRecordCreate(dnsRecord *from)
{
...
static dnsRecord *
dnsRecordAppend(dnsRecord **list,dnsRecord *pkt)
{
    if (!list || !pkt) return 0;
...
dnsParseName(dnsPacket *pkt,char **ptr,char *buf,int buflen,int pos,int level)
{
    ...
    while((len = *((*ptr)++)) != 0) {
      switch (len & 0xC0) {
       case 0xC0:
           if ((offset = ((len & ~0xC0)<<8)+(u_char)**ptr) >= pkt->buf.size) return -1;
           (*ptr)++;
      ...
      for (i = 0;i < len;i++) {
        if (--buflen <= 0) return -4;
        buf[pos++] = **ptr;
        (*ptr)++;
      }
      if (--buflen <= 0) return -5;
      buf[pos++] = '.';
    }
    buf[pos] = 0;
...
static dnsPacket *
dnsParseHeader(void *buf,int size)
{
    unsigned short *p;
    dnsPacket *pkt;
...
static dnsPacket *
dnsParsePacket(unsigned char *packet,int size)
{
    int i;
    dnsPacket *pkt;
    dnsRecord *rec;
    ...
    for (i = 0; i < pkt->qdcount; i++) {
      if (!(rec = dnsParseRecord(pkt,1))) goto err;
      dnsRecordAppend(&pkt->qdlist,rec);
    }
    if (!pkt->qdlist) goto err;
    for (i = 0; i < pkt->ancount; i++) {
      if (!(rec = dnsParseRecord(pkt,0))) goto err;
...
static void
dnsEncodeData(dnsPacket *pkt,void *ptr,int len)
{
...
static void
dnsEncodeGrow(dnsPacket *pkt,unsigned int size,char *proc)
{
    ...
    if (offset + size >= pkt->buf.allocated) {
      ...
      if (pkt->buf.rec) pkt->buf.rec = &pkt->buf.data[roffset];
    }
}

static dnsPacket *
dnsPacketCreateQuery(char *name,int type)
{
    dnsPacket *pkt = NULL;
...
static void
dnsPacketFree(dnsPacket *pkt,int type)
{
    if (!pkt) return;
...
static int
dnsPacketAddRecord(dnsPacket *pkt,dnsRecord **list,short *count,dnsRecord *rec)
{
